...
import { getSolveTime, startSolveTimer } from './pathUtils.js';
import {getSpeed, isPausedRef } from './pauseControl.js';
// Main control flags and variables
let racing = false;
let baseMaze = null;

// Start race button handler
...
// Clone grid to ensure separate solving instances
...
// Draw the maze in the given canvas container
...
// Run the selected solving algorithm on the cloned maze
  ...
  return new Promise((resolve) => {
    ...
    const sketch = (p) => {
      p.setup = async () => {
        ...
        try {
          console.log(`🛠️ Running solver ${solverNumber} on container ${containerId}`);
          ...
          if (done) {
            elapsed = getSolveTime();
            console.log(`✅ Solver ${solverNumber} done in ${elapsed.toFixed(2)}s`);
            p.redraw();
            resolve(elapsed);
          }
        } catch (err) {
...
// Return the correct maze algorithm object
...
// Display result after both solvers complete
